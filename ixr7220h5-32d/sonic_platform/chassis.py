...
class Chassis(ChassisBase):
    ...
    Nokia platform-specific Chassis class
        customized for the 7220 H5-32D platform.
    """
    ...
    def __init__(self):
        ...
        for index in range(PORT_START, PORT_START + PORT_END): # 1 ~ 34
            port_i2c_map = PORT_I2C_START + 4 * ((index-1) // 2) + ((index-1) % 2)
            port_eeprom_path = f"/sys/bus/i2c/devices/{port_i2c_map}-0050/eeprom"
    ...
    def get_revision(self):
        ...
        """
        #Revision is always 0 for 7220-IXR-H5-32D
        return str(0)
