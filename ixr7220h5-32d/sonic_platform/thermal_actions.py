try:
    ...
    from .thermal_infos import ChassisInfo, FanInfo
    import time
...
class ThermalRecoverAction(SetFanSpeedAction):
    ...
    """
    hook for the execute function, to fix the fan starting issue.
    """
    def hook(self,thermal_info_dict):
        SPEED_FOR_STARTING_FAN = 60 # x% speed
        TIME_FOR_STARTING_FAN = 15  # seconds
        if ChassisInfo.INFO_NAME not in thermal_info_dict:
            return

        chassis_info_obj = thermal_info_dict[ChassisInfo.INFO_NAME]
        chassis = chassis_info_obj.get_chassis()
        absent_fan_count = 0
        abnormal_fan_count = 0

        for fan in chassis.get_all_fans():
            if not fan.get_presence():
                absent_fan_count += 1
            if not fan.get_status():
                abnormal_fan_count += 1

        if absent_fan_count == 0 and abnormal_fan_count > 0:
            sonic_logger.log_warning(f"All the fans are present, set all the fans' speed to {SPEED_FOR_STARTING_FAN}% temporarily for {TIME_FOR_STARTING_FAN} seconds.")
            SetAllFanSpeedAction.set_all_fan_speed(thermal_info_dict, SPEED_FOR_STARTING_FAN)
            time.sleep(TIME_FOR_STARTING_FAN)
