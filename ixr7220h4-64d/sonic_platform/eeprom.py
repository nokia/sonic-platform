...
class Eeprom(TlvInfoDecoder):
    ...
    def __init__(self, is_psu, psu_index, is_fan, drawer_index):
        ...
        if self.is_sys_eeprom:
            ...
            self.base_mac = os.popen("ip link show eth0 | grep ether | awk '{print $2}'").read().rstrip('\n')
            self.model_str = "7220 IXR-H4"
            self.serial_number = os.popen("sudo decode-syseeprom -d | grep Serial | awk '{print $5}'").read().rstrip('\n')
