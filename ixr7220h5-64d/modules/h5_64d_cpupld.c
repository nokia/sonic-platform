...
#define DRIVER_NAME "h5_64d_cpupld"
...
static struct attribute *h5_64d_cpupld_attributes[] = {
...
static const struct attribute_group h5_64d_cpupld_group = {
    .attrs = h5_64d_cpupld_attributes,
};

static int h5_64d_cpupld_probe(struct i2c_client *client,
        const struct i2c_device_id *dev_id)
{
    ...
    dev_info(&client->dev, "Nokia-7220-IXR-H5-64D CPUCPLD chip found.\n");
    ...
    status = sysfs_create_group(&client->dev.kobj, &h5_64d_cpupld_group);
    if (status) {
    ...
    data->board_ver = cpld_i2c_read(data, BOARD_INFO_REG) & BOARD_INFO_REG_VER_MSK;
...
static void h5_64d_cpupld_remove(struct i2c_client *client)
{
    struct cpld_data *data = i2c_get_clientdata(client);
    sysfs_remove_group(&client->dev.kobj, &h5_64d_cpupld_group);
    kfree(data);
}

static const struct of_device_id h5_64d_cpupld_of_ids[] = {
    {
        .compatible = "nokia,h5_64d_cpupld",
        .data       = (void *) 0,
    },
    { },
};
MODULE_DEVICE_TABLE(of, h5_64d_cpupld_of_ids);

static const struct i2c_device_id h5_64d_cpupld_ids[] = {
    { DRIVER_NAME, 0 },
    { }
};
MODULE_DEVICE_TABLE(i2c, h5_64d_cpupld_ids);

static struct i2c_driver h5_64d_cpupld_driver = {
    .driver = {
        .name           = DRIVER_NAME,
        .of_match_table = of_match_ptr(h5_64d_cpupld_of_ids),
    },
    .probe        = h5_64d_cpupld_probe,
    .remove       = h5_64d_cpupld_remove,
    .id_table     = h5_64d_cpupld_ids,
    .address_list = cpld_address_list,
};

static int __init h5_64d_cpupld_init(void)
{
    return i2c_add_driver(&h5_64d_cpupld_driver);
}

static void __exit h5_64d_cpupld_exit(void)
{
    i2c_del_driver(&h5_64d_cpupld_driver);
}
...
module_init(h5_64d_cpupld_init);
module_exit(h5_64d_cpupld_exit);
