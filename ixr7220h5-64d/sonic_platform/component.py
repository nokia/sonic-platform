...
class Component(ComponentBase):
    ...
    def install_firmware(self, image_path):
        ...
        if self.name == "CPUPLD":
            ...
            if not os.path.isfile('/tmp/vme_h5_64d'):
                print("ERROR: the cpld upgrade tool /tmp/vme_h5_64d doesn't exist ")
                return False
            ...
            self.CPLD_UPDATE_COMMAND[2] = 'h5_64d_cpupld_refresh.vme'
            try:
                subprocess.run(self.CPLD_UPDATE_COMMAND, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                print(f"ERROR: Failed to upgrade CPLD: rc={e.returncode}")
            ...
            try:
                ...
                print(f"ERROR: Failed to upgrade CPLD: rc={e.returncode}")
            self.CPLD_UPDATE_COMMAND[2] = 'h5_64d_swpld2_refresh.vme'
            try:
                subprocess.run(self.CPLD_UPDATE_COMMAND, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                print(f"ERROR: Failed to upgrade CPLD: rc={e.returncode}")
            ...
            try:
                ...
                print(f"ERROR: Failed to upgrade CPLD: rc={e.returncode}")
            self.CPLD_UPDATE_COMMAND[2] = 'h5_64d_swpld3_refresh.vme'
            try:
                subprocess.run(self.CPLD_UPDATE_COMMAND, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                print(f"ERROR: Failed to upgrade CPLD: rc={e.returncode}")
            ...
            self.FPGA_UPDATE_COMMAND[5] = 'h5_64d_sysfpga_g.bit'
            try:
                subprocess.run(self.FPGA_UPDATE_COMMAND, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                print(f"ERROR: Failed to upgrade SysFPGA: rc={e.returncode}")
