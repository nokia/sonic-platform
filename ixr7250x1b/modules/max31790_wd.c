...
static ssize_t fan_wd_show(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	int val = 0;

	val = i2c_smbus_read_byte_data(client, MAX31790_REG_GLOBAL_CONFIG);
	if (val < 0) {
		dev_warn(&client->dev, "Fan Max31790 READ ERROR: reg(0x%02x) err %d\n", MAX31790_REG_GLOBAL_CONFIG, val);
	}

	return sprintf(buf, "%d\n", (val & 0x6)>>1);
}

static ssize_t fan_wd_store(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	struct max31790_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = to_i2c_client(dev);
	int status = 0;
	...
	status = kstrtou8(buf, 10, &usr_val);
	if (status != 0) {
		return status;
	}
	if (usr_val > 3) {
	...
	reg_val = i2c_smbus_read_byte_data(client, MAX31790_REG_GLOBAL_CONFIG);
	reg_val = reg_val & 0xF9;
	usr_val = usr_val << 1;
	status = i2c_smbus_write_byte_data(client, MAX31790_REG_GLOBAL_CONFIG, (reg_val | usr_val));
	if (status < 0) {
		dev_warn(&client->dev, "Fan Max31790 WRITE ERROR: reg(0x%02x) err %d\n", MAX31790_REG_GLOBAL_CONFIG, status);
	}

	return count;
}

static int fan_init(struct i2c_client *client)
{
	int i, rv;
	...
	rv = i2c_smbus_write_byte_data(client,
			MAX31790_REG_GLOBAL_CONFIG, (rv & 0xf9));
	if (rv < 0)
			return rv;

	for (i = 0; i < NR_CHANNEL; i++) {
		rv = i2c_smbus_write_byte_data(client,
				MAX31790_REG_PWMOUT(i), 0x80);
		if (rv < 0)
			return rv;
	}
...
// sysfs attributes 
static DEVICE_ATTR_RW(fan_wd);

static struct attribute *max31790_attributes[] = {
	&dev_attr_fan_wd.attr,
	NULL
};

static const struct attribute_group max31790_attr_group = {
	.attrs = max31790_attributes,
};
...
static int max31790_probe(struct i2c_client *client)
{
	...
	err = sysfs_create_group(&client->dev.kobj, &max31790_attr_group);
	if (err) {
...
MODULE_DESCRIPTION("MAX31790 sensor driver with WD");
MODULE_LICENSE("GPL");
